...
namespace saheki
{
    ...
    private: // pri data
        std::mt19937_64 mt;
    public: // fn
        template<typename min_type, typename max_type > static num_type number(const min_type& _min, const max_type& _max) 
        {
            return Get()._generate(static_cast<num_type>(_min), static_cast<num_type>(_max));
        }
        template<typename max_type > static num_type number(const max_type& _max) 
        {
